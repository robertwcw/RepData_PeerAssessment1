...
```
**
The total number of steps taken per day by the anonymous are illustrated in the histogram, where each column-bar in the graph represents the total steps count recorded each day. The mean and median of the steps count observations accumulated over the two months period as portrayed by the visual representation of the red and green horizontal lines overlaid on the histogram, hovering around the 10,000 mark indicate that the anonymous individual performed, on average ``r format(mean(actsum$Steps),nsmall=2)``, close to 10,000 steps per day in daily activities. The median value ``r format(median(actsum$Steps),nsmall=2)`` floating slightly above the 10,000 mark is mainly attributed by significantly greater number of steps taken by the anonymous individual on certain days during the period.
**
&nbsp;
...
i.e. start(m,n) 
where m = the number of the cycle depending on the frequency
      n = the first incident in that cycle
...
```{r ts.plot}
# create ts object from the activity data frame

# activity.ts <- ts(actsum$Mean,
#                   start = c(year(min(actsum$Date)), yday(min(actsum$Date))),
#                   # end = c(year(max(actsum$Date)), yday(max(actsum$Date))),
#                   frequency = 365
#                   )

# activity.ts <- ts(activity$Steps,
#                   start = c(1,1),
#                   # end = c(61,288),
#                   frequency = 12*24
#                   )
activity.ts <- ts(activity$Steps,
                  start = c(as.numeric(as_date(min(activity$Time))), 1),
                  # end = c(as.numeric(as_date(max(activity$Time))), 288),
                  frequency = 12*24
                  )

activity.agg.ts <- aggregate(activity.ts, FUN = mean, na.action = na.omit)
