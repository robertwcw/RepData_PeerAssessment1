...
Data structure of **activity** data set:
```{r}
str(activity)
```
...
There are missing columns-bar *( for dates Oct-01, Oct-08, Nov-01, Nov-04, Nov-09, Nov-10, Nov-14 and Nov-30 )* in the histogram due to the presence of missing values in the data set as mentioned.
...
```
The total number of steps taken per day by the anonymous are illustrated in the histogram, where each bar in the graph represents the total step count recorded each day. The mean and median of the step count observations accumulated over the two months period as portrayed by the visual representation of the red and green horizontal lines overlaid on the histogram, hovering around the 10,000 mark indicate that the anonymous individual performed, on average `r format(mean(actsum$Steps),nsmall=2)`, close to 10,000 steps per day in daily activities. The median value `r format(median(actsum$Steps),nsmall=2)` floating slightly above the 10,000 mark. However, the daily mean and daily median are about 1000 steps apart from each other or about 10% of the mean/median indicate there is some abnormality in the data set.
&nbsp;
...
The average daily activity pattern time-series plot shows the anonymous individual's daily activity pick up pace starting 0600 hour lasted for 13 hours, begin to wind down from 1900 hour onward and the step count slow down gradually to negligible level at around 2130 hour continuing forward. There is a sharp spike (indicated by the red color vertical line) in the average activity protruded at ``r sprintf("%04.0f", h[1])`` hour to ``r print(h[2])`` steps, which is double the average step count of 100 for most part of the day.
&nbsp;

```{r}
par(mfrow = c(1,2))
boxplot(activity.ts[,2], ylab = "intreval average step count")
hist(activity.ts[,2], main = NULL, xlab = "interval average step count")
```

As shown in the Boxplot (left) and histogram (right) above, least number of observations data points revolving around the average 200-step count mark affirm the unusual sharp spike exhibited in the Average Daily Activity Pattern time series graph plot is the result of this data circumstance.
&nbsp;
...
The present of missing values will induce biased results in the analysis.  Henceforth, K-Nearest Neighbour (knn) algorithm is employed to impute missing values in the data set, as knn imputation method is well accepted in the community among the practitioners.
...
5-numbers summary of imputed **activity** data set shows that missing values had been imputed.  
...
The histogram for **Number of Steps Taken Per Day** plotted using imputed **activity** data set, compared to the first histogram of the same based on pre-imputed data, proximity of both the mean value `r format(mean(actsum$Steps),nsmall=2)` and median value `r format(median(actsum$Steps),nsmall=2)` are now become fairly close as indicated by the red horizontal line and green horizontal line respectively. Note the mean line is now moved up the scale to above the 10000-step mark in contrast to the median line remain unchanged, the shift in central values to become much closer to each other explain the imputed data set had attained improved symmetrical distribution.
&nbsp;
...
dow <- if_else(wday(as_date(activity.imputed$Date)) %in% c(7,1),"weekend","weekday")
