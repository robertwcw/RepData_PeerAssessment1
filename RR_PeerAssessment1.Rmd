...
* **activity** variable data type.
  + **steps**: integer class vector.
  + **date**: character class vector.
  + **interval**: integer class vector.
  
Take note that the variable **date** shall inherits date class vector instead of the current character class vector for which date data values are stored as character string in the **activity** data set. Time-series related analysis requires that the **date** variable be transformed into formal date class vector to warrant robust analysis.
...
The 5-numbers summary of **activity** data set shows there are `r na.count` missing values (coded as NA), representing approx. `r percent(na.ratio)` missing values out of the **activity** data set. The ratio of missing values considered significant in this case which can skewed the outcomes of downstream analysis.
...
There are missing bar-columns *( for dates Oct-01, Oct-08, Nov-01, Nov-04, Nov-09, Nov-10, Nov-14 and Nov-30 )* in the histogram plotted below due to the presence of missing values in the data set as mentioned.

```{r steps.hist.plot}

h <- hist(as.Date(actsum$Date), breaks = "days", freq = FALSE, plot = FALSE)
...
Mean value (`r format(steps.mean,nsmall=2)`) and Median value (`r format(steps.median,nsmall=2)`) of the **Number of Steps Taken Per Day** are indicated by the visual representation of both red and green horizontal lines overlaid on the histogram above.  
