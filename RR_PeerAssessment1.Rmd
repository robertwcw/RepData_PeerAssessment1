...
The visual representation (above) of the *average daily activity data* shows the least number (lowest frequency) of observations data having step count close to 200 average step count as depicted in the histogram. Boxplot graph also confirms similar event where there are two data points hovering around the 200 average step count mark. Therefore, we can affirm that the unusual huge spike in the Average Daily Activity Pattern time series graph is caused by this data circumstance.
&nbsp;

### Imputing Missing Values

As reported earlier, there are `r na.count` missing values (coded as NA), constituting `r percent(na.ratio)` out of the total observation data in the **activity** data set. 
...
The present of missing values will induce biased results in the analysis.  Henceforth, K-Nearest Neighbour (knn) algorithm is employed to impute the missing values in the data set, as knn imputation method is well accepted in the community among the practitioners.
...
5-numbers summary of the imputed **activity** data set shows that the missing values had been imputed.  
