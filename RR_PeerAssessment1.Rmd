...
activity <- activity %>%
                mutate(activity, Date = as.Date(Date), Time = date.time) %>%
                arrange(Time)
...
```{r ts.plot, include=FALSE, echo=FALSE, results="hide"}
# time series plot for daily activity pattern

# create time series object from 5-min interval seasonality data
activity.ts <- ts(activity,
                  start = c(1,1),
                  frequency = 12*24
                  )

par(cex.axis = 0.8, las = 2, ann = FALSE)
plot(activity.ts[,1] ~ activity.ts[,3], type = "l", col = "royalblue3")
h <- aggregate(activity.ts[,1] ~ activity.ts[,3], FUN = mean)
lines(h, type = "l", lwd = 1.5, col = pal[6])
abline(v = h[which(h[,2] == max(h[,2])),1], col = pal[4])
title(main = "Average Daily Activities Pattern", 
      # sub = "( data points plotted @ 5-min interval )",
      xlab = "Time Interval", 
      ylab = "Steps Count",
