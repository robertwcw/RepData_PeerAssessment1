...
```
**
The total number of steps taken per day by the anonymous are illustrated in the histogram, where each bar-column in the graph represents the total steps count recorded each day. The mean value ``r format(mean(actsum$Steps),nsmall=2)`` and median value ``r format(median(actsum$Steps),nsmall=2)`` of the steps count observations accumulated over the two months period as portrayed by the visual representation of the red and green horizontal lines overlaid on the histogram, hovering closely around the 10,000 mark indicate that the anonymous individual performed, on average, `10,000` steps per day in daily activities during the months of October and November, 2012.  
**
&nbsp;
